...
class Neuron:
  ...
  def compute(self,x):
    ...
    '''
    # TODO
    #self.y = numpy.math.sqrt((self.weights[0]-x[0])**2+(self.weights[1]-x[1])**2)
    self.y = numpy.math.sqrt((self.weights[0]-x[0])**2+(self.weights[1]-x[1])**2)
  ...
  def learn(self,eta,sigma,posxbmu,posybmu,x):
    ...
    dis = numpy.math.sqrt( (self.posx-posxbmu)**2 + (self.posy-posybmu)**2 )
    self.weights[:] += (eta * numpy.exp(-(dis)**2/(2*sigma**2))) * (x-self.weights)
...
class SOM:
  ...
  def euclideanDistance(self,n1,n2):
    return numpy.sqrt(numpy.sum((n1.weights - n2.weights) ** 2))
  
  def neuron_cohesion(self, posx, posy):
    total_distance = 0
    count = 0
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            if not (dx == 0 and dy == 0):
                ...
                if 0 <= x < self.gridsize[0] and 0 <= y < self.gridsize[1]:
                    total_distance += self.euclideanDistance(self.map[posx][posy], self.map[x][y])
                    count += 1
    return total_distance / count

  def average_cohesion(self):
    total_cohesion = 0
    for posx in range(self.gridsize[0]):
      for posy in range(self.gridsize[1]):
        total_cohesion += self.neuron_cohesion(posx, posy)
    return total_cohesion / (self.gridsize[0] * self.gridsize[1])
...
#  plt.show()
  values = numpy.array([])
  cohesionValues = numpy.array([])


for SIGMA in SIGMAS:

  print(f"Cohesion moyenne avant apprentissage: {SIGMA} val : {network.average_cohesion()}")
  ...
  errorVect = network.MSE(samples)
  cohesionVal = network.average_cohesion()
  print(f"MSE pour largeur {SIGMA} : {errorVect}")
  values = numpy.append(values,errorVect)
  print(f"Cohesion moyenne apres apprentissage: {SIGMA} val : {cohesionVal}")
  cohesionValues = numpy.append(cohesionValues ,cohesionVal)
  



'''plt.plot(SIGMAS, values, 'o-', label='EQV')
plt.plot(SIGMAS, cohesionValues, 'o-', label='Cohesion moyenne')
plt.xlabel("Largeur Voisinage Gaussien")
plt.ylabel('Valeurs')
plt.show()'''
    
  #print("*"*10)
  #print("La moyenne de l'erreur de quantification vectorielle moyenne est " + str(values.mean()))
